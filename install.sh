...
cd libvpx && \
./configure --enable-shared --disable-examples --disable-unit-tests --enable-vp9-highbitdepth --as=yasm && \
make && \
sudo make install
...
mkdir -p aom_build && \
cd aom_build && \
cmake -G "Unix Makefiles" -DENABLE_SHARED=on -DENABLE_NASM=on ../aom && \
make && \
sudo make install
...
cd kvazaar && \
./autogen.sh && \
./configure --enable-shared --enable-shared && \
make && \
sudo make install
